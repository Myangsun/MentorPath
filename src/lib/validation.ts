...
export const studentProfileSchema = z.object({
  ...
  graduationYear: z.number({ invalid_type_error: 'Graduation year must be a number' }).int('Must be a whole number').min(2000, 'Year must be 2000 or later').max(2030, 'Year must be 2030 or earlier').optional(),
